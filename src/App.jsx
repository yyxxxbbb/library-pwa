...
import { updateSeatsLayout } from './api/seatApi';
...
function App() {
  ...
  const handleDeleteSeat = async (seatId) => {
    try {
      ...
      console.log(`✅ [${seatId}] 좌석이 DB에서 성공적으로 삭제되었습니다.`);
    } catch (error) {
  ...
  // 💡 [추가] 층별 공통 아이콘 (파란색 사각형 안 4개 작은 창문 모양)
  const FacilityIcon = () => (
    <div style={{ width: '60px', height: '60px', background: '#eff6ff', borderRadius: '16px', display: 'flex', justifyContent: 'center', alignItems: 'center', flexShrink: 0 }}>
  ...
  return (
    <div style={{ padding: isSmallMobile ? '10px' : '20px', width: '100%', maxWidth: '1300px', margin: '0 auto', boxSizing: 'border-box', fontFamily: 'sans-serif', background: '#f8fafc', minHeight: '100vh', position: 'relative', paddingBottom: myTicket && (viewMode === 'HOME' || viewMode === 'MAP') ? '120px' : '30px' }}>
      ...
      <header style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', marginBottom: '20px', background: '#fff', padding: '15px 25px', borderRadius: '20px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', width: '100%', boxSizing: 'border-box', justifyContent: 'space-between', alignItems: 'center', gap: '15px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'center', width: isMobile ? '100%' : 'auto', flexWrap: 'wrap' }}>
        ...
        <>
          {viewMode === 'HOME' && (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: isMobile ? 'column-reverse' : 'row', gap: '20px', marginBottom: '25px' }}>
                <div style={{ 
                  flex: 1.5, 
                  display: 'grid', 
                  ...
                  {[
                    ...
                    return (
                      ...
                      }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', gap: '8px', padding: '25px 10px', borderRadius: '16px', background: '#fff', transition: 'all 0.2s ease', border: '1px solid #e2e8f0', boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}
                      onMouseEnter={(e) => { e.currentTarget.style.background = '#f8fafc'; e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 6px 15px rgba(0,0,0,0.06)'; }}
                      onMouseLeave={(e) => { e.currentTarget.style.background = '#fff'; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 2px 8px rgba(0,0,0,0.03)'; }}
                      >
                        <div style={{ color: '#2563eb' }}>{item.svg}</div>
                        <span style={{ fontSize: '0.9rem', fontWeight: '800', color: '#1e293b', marginTop: '4px' }}>{item.label}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: isMobile ? 'row' : 'column', gap: '15px' }}>
                  <button onClick={() => { setIsBookingMode(true); setSelectedSeat(null); setViewMode('MAP'); }} style={{ flex: 1, padding: isMobile ? '15px' : '20px', background: '#2563eb', color: '#fff', borderRadius: '20px', fontSize: '1.2rem', fontWeight: '900', border: 'none', cursor: 'pointer', boxShadow: '0 8px 20px rgba(37, 99, 235, 0.2)', transition: 'transform 0.1s' }} onMouseDown={e => e.currentTarget.style.transform = 'scale(0.98)'} onMouseUp={e => e.currentTarget.style.transform = 'scale(1)'}>좌석 예약하러 가기</button>
                  <button onClick={() => setShowIdQR(true)} style={{ flex: 1, padding: isMobile ? '15px' : '20px', background: '#0f172a', color: '#fff', borderRadius: '20px', fontSize: '1.2rem', fontWeight: '900', border: 'none', cursor: 'pointer', boxShadow: '0 8px 20px rgba(15, 23, 42, 0.2)', transition: 'transform 0.1s' }} onMouseDown={e => e.currentTarget.style.transform = 'scale(0.98)'} onMouseUp={e => e.currentTarget.style.transform = 'scale(1)'}>내 모바일 출입증</button>
                </div>
              </div>

              {/* 💡 [복구 완료] 시설 안내 부분 (아이콘 + 세부 공간별 텍스트 추가) */}
              <div style={{ marginBottom: '25px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
                  ...
                  {/* 1층 도서관 */}
                  <div style={{ background: '#fff', borderRadius: '24px', padding: '25px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 10px rgba(0,0,0,0.02)', height: '100%' }}>
                    ...
                    <p style={{ margin: '0 0 20px 0', fontSize: '0.85rem', color: '#64748b', fontWeight: '700' }}>수용 인원: 72명 | 테이블 5개 | 좌석 68석</p>
                    
                    <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flex: 1 }}>
                      <FacilityIcon />
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        <div>
                          <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: '900', color: '#1e293b' }}>일반 열람석 (입구/창가/벽면/계단)</p>
                          <p style={{ margin: '2px 0 0 0', fontSize: '0.75rem', color: '#64748b' }}>총 60석</p>
                        </div>
                        <div>
                          <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: '900', color: '#1e293b' }}>스터디룸 1 (101-A)</p>
                          <p style={{ margin: '2px 0 0 0', fontSize: '0.75rem', color: '#64748b' }}>최대 6명 / 좌석 8석</p>
                        </div>
                        <div>
                          <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: '900', color: '#1e293b' }}>스터디룸 2 (101-B 좌식)</p>
                          <p style={{ margin: '2px 0 0 0', fontSize: '0.75rem', color: '#64748b' }}>최대 6명 (좌석 산정 제외)</p>
                        </div>
                      </div>
                    </div>

                    <button onClick={() => { setActiveFloor('1층'); setIsBookingMode(false); setSelectedSeat(null); setViewMode('MAP'); mapRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }} style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0', background: '#fff', color: '#2563eb', fontWeight: '800', fontSize: '0.9rem', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', transition: '0.2s' }} onMouseEnter={(e) => e.currentTarget.style.background = '#f1f5f9'} onMouseLeave={(e) => e.currentTarget.style.background = '#fff'}>배치도 보기 →</button>
                  </div>
                  ...
                  <div style={{ background: '#fff', borderRadius: '24px', padding: '25px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 10px rgba(0,0,0,0.02)', height: '100%' }}>
                    ...
                    <p style={{ margin: '0 0 20px 0', fontSize: '0.85rem', color: '#64748b', fontWeight: '700' }}>수용 인원: 40명 | 테이블 6개 | 좌석 42석</p>
                    
                    <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flex: 1 }}>
                      <FacilityIcon />
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        <div>
                          <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: '900', color: '#1e293b' }}>집중 열람석 (창가/중앙/계단입구)</p>
                          <p style={{ margin: '2px 0 0 0', fontSize: '0.75rem', color: '#64748b' }}>총 32석</p>
                        </div>
                        <div>
                          <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: '900', color: '#1e293b' }}>스터디룸 (101-C)</p>
                          <p style={{ margin: '2px 0 0 0', fontSize: '0.75rem', color: '#64748b' }}>최대 4명 / 좌석 5석</p>
                        </div>
                        <div>
                          <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: '900', color: '#1e293b' }}>스터디룸 (101-D)</p>
                          <p style={{ margin: '2px 0 0 0', fontSize: '0.75rem', color: '#64748b' }}>최대 4명 / 좌석 5석</p>
                        </div>
                      </div>
                    </div>

                    <button onClick={() => { setActiveFloor('2층'); setIsBookingMode(false); setSelectedSeat(null); setViewMode('MAP'); mapRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }} style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0', background: '#fff', color: '#2563eb', fontWeight: '800', fontSize: '0.9rem', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', transition: '0.2s' }} onMouseEnter={(e) => e.currentTarget.style.background = '#f1f5f9'} onMouseLeave={(e) => e.currentTarget.style.background = '#fff'}>배치도 보기 →</button>
                  </div>
                  ...
                  <div style={{ background: '#fff', borderRadius: '24px', padding: '25px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 10px rgba(0,0,0,0.02)', height: '100%' }}>
                    ...
                    <p style={{ margin: '0 0 20px 0', fontSize: '0.85rem', color: '#64748b', fontWeight: '700' }}>수용 인원: 54명 | 테이블 15개 | 좌석 54석</p>
                    
                    <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flex: 1 }}>
                      <FacilityIcon />
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        <div>
                          <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: '900', color: '#1e293b' }}>입구 좌/우측 구역 (대칭 구조)</p>
                          <p style={{ margin: '2px 0 0 0', fontSize: '0.75rem', color: '#64748b' }}>쿠션석·1인석·창가석 총 34석</p>
                        </div>
                        <div>
                          <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: '900', color: '#1e293b' }}>중앙 테이블 구역</p>
                          <p style={{ margin: '2px 0 0 0', fontSize: '0.75rem', color: '#64748b' }}>테이블 2개 / 총 10석</p>
                        </div>
                        <div>
                          <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: '900', color: '#1e293b' }}>중앙 창가 구역</p>
                          <p style={{ margin: '2px 0 0 0', fontSize: '0.75rem', color: '#64748b' }}>테이블 1개 / 총 10석</p>
                        </div>
                      </div>
                    </div>

                    <button onClick={() => { setActiveFloor('4층'); setIsBookingMode(false); setSelectedSeat(null); setViewMode('MAP'); mapRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }} style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0', background: '#fff', color: '#3b82f6', fontWeight: '800', fontSize: '0.9rem', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', transition: '0.2s' }} onMouseEnter={(e) => e.currentTarget.style.background = '#f1f5f9'} onMouseLeave={(e) => e.currentTarget.style.background = '#fff'}>배치도 보기 →</button>
                  </div>
          ...
          {viewMode === 'MAP' && (
            <>
              ...
              <FloorMap
                ...
                onUpdateSeats={updateSeatsLayout}
          ...
          {myTicket && (
            <div style={{ position: 'fixed', bottom: '25px', left: '50%', transform: 'translateX(-50%)', width: '92%', maxWidth: '420px', background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(10px)', borderRadius: '30px', padding: '16px 24px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)', border: '1px solid rgba(37, 99, 235, 0.2)', zIndex: 9998 }}>
              
              {myTicket.status !== 'OCCUPIED' ? (
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    <div>
                      <span style={{ background: '#fef08a', color: '#ca8a04', padding: '4px 10px', borderRadius: '12px', fontSize: '0.75rem', fontWeight: '900' }}>
                        {myTicket.isImmediate ? "⚡ 현장 대기" : (myTicket.date === format(new Date(), 'yyyy-MM-dd') ? "🎫 오늘 예약" : "🎫 사전 예약")}
                      </span>
                      <h3 style={{ margin: '6px 0 0 0', fontSize: '1.2rem', fontWeight: '900' }}>
                        {myTicket.seatId}
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <p style={{ margin: 0, color: '#64748b', fontSize: '0.75rem', fontWeight: '700' }}>이용 시간</p>
                      <p style={{ margin: 0, color: '#2563eb', fontSize: '1rem', fontWeight: '900' }}>{myTicket.startTime} ~ {myTicket.endTime}</p>
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={() => setShowIdQR(true)} style={{ flex: 2, padding: '12px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '16px', fontWeight: '900', cursor: 'pointer', fontSize: '0.9rem', boxShadow: '0 4px 12px rgba(37,99,235,0.2)' }}>QR 인증하기</button>
                    <button onClick={handleCancel} style={{ flex: 1, padding: '12px', background: '#fee2e2', color: '#dc2626', border: 'none', borderRadius: '16px', fontWeight: '900', cursor: 'pointer', fontSize: '0.9rem' }}>취소</button>
                  </div>
                </>
              ) : (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <p style={{ margin: '0 0 4px 0', color: '#2563eb', fontSize: '0.8rem', fontWeight: '900' }}>👤 {isAdmin ? '관리자' : (currentUserData?.name || user?.displayName || user?.email?.split('@')[0])}님 사용중</p>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
                      <h3 style={{ margin: 0, fontSize: '1.3rem', fontWeight: '900', color: '#0f172a' }}>{myTicket.seatId}</h3>
                      <p style={{ margin: 0, color: '#64748b', fontSize: '0.85rem', fontWeight: '700' }}>~ {myTicket.endTime}</p>
                    </div>
                  </div>
                  <button onClick={handleCheckout} style={{ padding: '12px 20px', background: '#475569', color: '#fff', border: 'none', borderRadius: '16px', fontWeight: '900', cursor: 'pointer', fontSize: '0.9rem', boxShadow: '0 4px 10px rgba(0,0,0,0.1)' }}>퇴실하기</button>
                </div>
              )}
            </div>
          )}
      ...
      <div style={{ position: 'fixed', bottom: myTicket ? '120px' : '30px', right: '30px', zIndex: 999999, transition: 'bottom 0.3s ease' }}>
        <FloatingChatbot />
      </div>
    </div>
  );
