    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '5px' }}>
        ...
        {calendarDays.map((day, i) => (
          <button key={i} disabled={isPast(day) && !isSameDay(day, new Date())} onClick={() => { setSelectedDate(day); setShowFullCalendar(false); }} style={{ padding: '12px 0', border: 'none', borderRadius: '10px', cursor: 'pointer', transition: '0.2s', background: isSameDay(day, selectedDate) ? '#2563eb' : (isSameMonth(day, monthStart) ? '#f8fafc' : 'transparent'), color: isSameDay(day, selectedDate) ? '#fff' : (isPast(day) && !isSameDay(day, new Date()) ? '#cbd5e1' : '#1e293b'), fontWeight: '800', fontSize: '0.95rem', opacity: isSameMonth(day, monthStart) ? 1 : 0.3 }}>
            {format(day, 'd')}
          </button>
        ))}
      </div>
    ...
    <>
          ...
          <div style={{ background: '#fff', padding: '30px', borderRadius: '24px', width: '100%', maxWidth: '400px', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
              {['자리 쓰레기 방치', '도서관 기물 파손', '장시간 이석 및 짐만 두기', '심한 소음 유발'].map(issue => {
                ...
                return (
                  <button key={issue} onClick={() => setSelectedIssues(isChecked ? selectedIssues.filter(i=>i!==issue) : [...selectedIssues, issue])} style={{ padding: '12px', borderRadius: '12px', border: `2px solid ${isChecked ? '#dc2626' : '#e2e8f0'}`, background: isChecked ? '#fef2f2' : '#fff', color: isChecked ? '#dc2626' : '#475569', fontWeight: '800', cursor: 'pointer', textAlign: 'left' }}>{issue}</button>
                )
              })}
            </div>
            <textarea placeholder="추가 사유 입력 (선택)" value={textContent} onChange={e=>setTextContent(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', marginBottom: '15px', resize: 'none', height: '60px' }} />
            <input type="file" accept="image/*,video/*" multiple onChange={handleFileChange} style={{ marginBottom: '25px', fontSize: '0.85rem' }} />
      ...
      {showAppealPopup && reportTarget && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 999999, padding: '20px' }}>
          <div style={{ background: '#fff', padding: '30px', borderRadius: '24px', width: '100%', maxWidth: '400px' }}>
            ...
            <textarea placeholder="억울한 사유를 자세히 설명해 주세요." value={textContent} onChange={e=>setTextContent(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', marginBottom: '15px', resize: 'none', height: '100px' }} />
      ...
      {!showFullCalendar && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(5px)', zIndex: 999, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px', boxSizing: 'border-box' }}>
          <div style={{ background: '#fff', padding: '30px', borderRadius: '24px', width: '100%', maxWidth: '450px', maxHeight: '90vh', overflowY: 'auto' }}>
