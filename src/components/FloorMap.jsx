            ...
            {mapSeats.map(seat => {
              ...
              if (!isEditMode) {
                if (seat.status === 'RESERVED') {
                  ...
                  borderColor = '#eab308';
                  textColor = '#854d0e';
                  statusText = isAdmin ? (seat.studentNo || '예약중') : '예약중';
                } else if (seat.status === 'OCCUPIED') {
                  seatBg = '#bfdbfe';
                  borderColor = '#3b82f6';
                  textColor = '#1e3a8a';
                  statusText = isAdmin ? (seat.studentNo || '사용중') : '사용중';
              ...
              return (
                ...
                >
                  <div 
                    ...
                    // 💡 [핵심 수정] 관리자가 아니면 'DISABLED' 좌석은 클릭해도 반응하지 않음 (setSelectedSeat 실행 안 함)
                    onClick={() => {
                      if (isEditMode) {
                        setSelectedEditId(seat.id);
                      } else if (seat.status === 'DISABLED' && !isAdmin) {
                        return; // 일반 유저는 비활성화 좌석 클릭 무시
                      } else {
                    ...
                    <div style={{ 
                      ...
                      background: isEditMode ? 'rgba(255,255,255,0.85)' : seatBg, 
                      borderRadius: '12px', 
                      border: isSelected ? '4px solid #2563eb' : `2px solid ${borderColor}`, 
                      display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', 
                      cursor: (seat.status === 'DISABLED' && !isAdmin) ? 'not-allowed' : 'pointer', // 💡 마우스 커서도 금지 표시로 변경
                      boxShadow: isSelected ? '0 10px 25px rgba(37, 99, 235, 0.4)' : '0 2px 5px rgba(0,0,0,0.05)', 
                      transform: isSelected ? 'scale(1.15)' : 'scale(1)', 
                      transition: 'all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)', 
                      backdropFilter: isEditMode ? 'blur(4px)' : 'none' 
                    }}>
                      <span style={{ fontWeight: '900', fontSize: '0.9rem', pointerEvents: 'none', color: textColor }}>{seat.label}</span>
                      
                      {!isEditMode && statusText && (
                        <span style={{ fontSize: '0.65rem', fontWeight: '900', color: textColor, marginTop: '2px', pointerEvents: 'none' }}>
                          {statusText}
                        </span>
                      )}
                    </div>
