...
export default function EventDetail({ event, onBack }) {
  ...
  if (!event) {
    return (
      <div style={{ background: "#fff", padding: "40px", borderRadius: "25px", textAlign: "center", boxShadow: "0 5px 20px rgba(0,0,0,0.05)" }}>
        <p style={{ color: "#94a3b8", fontWeight: 700 }}>이벤트 정보를 불러올 수 없습니다.</p>
  ...
  // ====================================================================
  // 💡 [버그 수정] 오늘 날짜를 기준으로 이벤트 상태를 계산하는 로직
  // ====================================================================
  const todayStr = new Date().toISOString().split("T")[0];

  const getEventStatus = (e) => {
    if (!e.isActive) return "ENDED"; // 관리자가 직접 끈 경우
    if (e.endDate && e.endDate < todayStr) return "ENDED"; // 기간 종료
    if (e.startDate && e.startDate > todayStr) return "UPCOMING"; // 시작 전 (진행 예정)
    return "ACTIVE"; // 진행 중
  };

  const status = getEventStatus(event);
  let badgeBg = "#f1f5f9";    // 기본: 종료
