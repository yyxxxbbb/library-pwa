...
export default function AdminDashboard() {
  ...
  // 💡 [추가] 챗봇 공지사항 크롤링 및 학습 로직 (시뮬레이션/API 연결부)
  const handleCrawlNotices = async () => {
    if (!window.confirm("외부 도서관 공지사항 및 최신 규정을 크롤링하여 AI 챗봇에 학습시키겠습니까?")) return;
    
    setIsCrawling(true);
    try {
      // (여기에 실제 크롤링 봇 API 통신 또는 Firebase Functions 호출 코드가 들어갑니다)
      // 시연을 위해 2.5초 대기 후 완료 알림을 띄우는 로직으로 구성
      await new Promise(resolve => setTimeout(resolve, 2500));
      
      alert("✅ 크롤링 및 AI 챗봇 지식 베이스 업데이트가 완료되었습니다!\n이제 챗봇이 최신 도서관 규정을 기반으로 답변합니다.");
    } catch (error) {
      alert("🚨 크롤링 서버와 통신 중 오류가 발생했습니다.");
      console.error(error);
    } finally {
      setIsCrawling(false);
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: window.innerWidth < 600 ? '15px' : '30px', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ background: '#fff', padding: window.innerWidth < 600 ? '20px' : '40px', borderRadius: '25px', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', width: '100%', boxSizing: 'border-box' }}>
        ...
        {/* 💡 [추가] 챗봇 공지사항 크롤링 가동 버튼 (우측 정렬) */}
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '15px' }}>
          <button 
            onClick={handleCrawlNotices}
            disabled={isCrawling}
            style={{ 
              padding: '12px 20px', 
              background: isCrawling ? '#c084fc' : '#9333ea', 
              color: '#fff', 
              border: 'none', 
              borderRadius: '12px', 
              fontWeight: '900', 
          ...
          >
            {isCrawling ? '⏳ 외부 문서 수집 및 AI 학습 중...' : '🤖 외부 공지사항 크롤링 봇 가동'}
          </button>
        </div>
